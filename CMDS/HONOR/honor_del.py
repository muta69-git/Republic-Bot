async def run(discord, message, args, admin_role, embed_colors, qik):
  ...
  if _hasr_(admin_role):
    if len(args) <= 1:
      invalid_arg_index_embed = discord.Embed(title = "*invalid arg index:*", description = f"*`provided args: {len(args)}`\nproper use:  `!honor.del <@user (arg1)> <int (arg2)>`*", color = embed_colors["err"]);
